...
// NEXT/PREVIOUS LINKS
		$next= mysqli_query($con,"SELECT id FROM mugallery WHERE id = (select min(id) from mugallery where id > '$id') LIMIT 1");
		while ($row = mysqli_fetch_array($next)){
			$idNext =  $row['id'];

		}
		$prev= mysqli_query($con,"SELECT id FROM mugallery WHERE id = (select max(id) from mugallery where id < '$id') LIMIT 1");
...
<div class="row">
	<div class="col-md-12">
		...
		$result2 = mysqli_query($con, "SELECT * FROM mugallery JOIN users on users.user_id = mugallery.author_id WHERE id = '$id'") or die(mysqli_error($con));
		?>
		...
		<div class="display-info text-primary display-description">
